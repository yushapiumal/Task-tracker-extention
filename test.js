...
const taskModal = document.getElementById("task-modal");
  ...
  function showTaskScreen(username) {
      ...
      .then((data) => {
        if (data && data.tasks) {
          chrome.storage.local.get(['activeTask'], (result) => {
            ...
            // If there is an active task, patch its startTime into the task array
            tasks = data.tasks.map((task) => {
              if (active && task._id === active.taskId) {
                ...
                return {
                  ...task,
                  status: 'inprogress',
                  startTime: active.startTime // <-- this fixes your NaN!
                };
              }
              return task;
            });
  ...
  function renderTasks(tasks) {
    console.log('renderTasks: Rendering tasks:', tasks, 'activeTaskId:', activeTaskId);
    taskList.innerHTML = '';
    ...
    tasks.forEach((task) => {
      ...
      let durationDisplay = task.duration ? `<span class="task-duration">Worked: ${task.duration}</span>` : '';
      ...
      li.innerHTML = `
        <div class="task-content">
          ...
          <div class="task-timer ${isActive ? '' : 'hidden'}" id="timer-${task._id}">
      ...
      // ✅ NEW: Setup timer tick if task is active
      if (isActive) {
        const timerDisplay = li.querySelector(`#timer-${task._id} .timer-display`);
      ...
      li.querySelector('.task-action-btn').addEventListener('click', () => {
        console.log('Task button clicked, taskId:', task._id, 'isActive:', isActive);
        if (isActive) {
  ...
  function handleStartTask(taskId) {
    console.log('handleStartTask: taskId:', taskId, 'activeTaskId:', activeTaskId);
    if (activeTaskId) {
        ...
        chrome.runtime.sendMessage(
          ...
          (response) => {
            if (chrome.runtime.lastError) {
              console.error('Error sending START_TASK_REMINDER:', chrome.runtime.lastError.message);
            } else {
  ...
  function startTimer(taskId, storedStartTime) {
    console.log('startTimer: taskId:', taskId, 'storedStartTime:', storedStartTime);
    if (timers[taskId]) {
      clearInterval(timers[taskId].interval);
      delete timers[taskId];
    }
  
    const timerDisplay = document.querySelector(`#timer-${taskId} .timer-display`);
    const taskElement = document.querySelector(`.task-item[data-id="${taskId}"] .task-timer`);
    ...
    const startTime = storedStartTime instanceof Date ? storedStartTime : new Date(storedStartTime);
    if (isNaN(startTime.getTime())) {
...
function handleSearchAddClick() {
  ...
  const exists = tasks.some(task =>
    task.task.toLowerCase() === searchTerm.toLowerCase()
  );
...
async function loadClients() {
  ...
  try {
    const response = await fetch(`https://dtbl.go.digitable.io:3004/neverdone/api/settings/get-clients`, {
      credentials: "include",
    });
...
async function loadProjects() {
  ...
  try {
    const response = await fetch(`https://dtbl.go.digitable.io:3004/neverdone/api/settings/get-projects`, {
      credentials: "include",
    });
    if (!response.ok) throw new Error(`HTTP error ${response.status}`);
...
function saveTaskFromModal() {
  ...
  addTask(taskName, priority, status, endDate, notes, clientName, projectName);
